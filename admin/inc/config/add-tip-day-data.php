...
if (isset($_POST['tip_name']) && !empty($action->database->text_filter($_POST['tip_name']))) {
    $tip_name = $action->database->text_filter($_POST['tip_name']);
} else {
    $json['status'] = 101;
    $json['msg'] = 'Oops!! Enter Tips';
}
...
if (!isset($json['status'])) {
    ...
    if (isset($_POST['hidden_id']) && !empty($action->database->text_filter($_POST['hidden_id']))) {
        ...
        if (!isset($json['status']) && empty($hidden_id)) {
            ...
            $field['tip_text'] = $tip_name;
            $field['date'] = $date;
            $insert = $action->database->insert_query("tbl_tip_day", $field);
            if ($insert) {
                $json['status'] = 100;
