...
if (isset($_POST['words_name']) && !empty($action->database->text_filter($_POST['words_name']))) {
    $words_name = $action->database->text_filter($_POST['words_name']);
} else {
    $json['status'] = 101;
    $json['msg'] = 'Oops!! Enter Words';
}
...
if (!isset($json['status'])) {
    ...
    if (isset($_POST['hidden_id']) && !empty($action->database->text_filter($_POST['hidden_id']))) {
        ...
        if (!isset($json['status']) && empty($hidden_id)) {
            ...
            $field['words'] = $words_name;
            $field['date'] = $date;
            $insert = $action->database->insert_query("tbl_word_day", $field);
            if ($insert) {
                $json['status'] = 100;
