...
if (isset($_POST['videos_link']) && !empty($action->database->text_filter($_POST['videos_link']))) {
    $videos_link = $action->database->text_filter($_POST['videos_link']);
} else {
    $json['status'] = 101;
    $json['msg'] = 'Oops!! Select Videos Links';
}

if (isset($_POST['videos_title']) && !empty($action->database->text_filter($_POST['videos_title']))) {
    $videos_title = $action->database->text_filter($_POST['videos_title']);
} else {
    $json['status'] = 101;
    $json['msg'] = 'Oops!! Select Videos Title';
}
...
if (!isset($json['status'])) {
    ...
    if (isset($_POST['hidden_id']) && !empty($action->database->text_filter($_POST['hidden_id']))) {
        ...
        if (!isset($json['status']) && empty($hidden_id)) {
            $field['video_title'] = $videos_title;
            $field['video_link'] = $videos_link;
            $field['question'] = $question;
            $field['config'] = $config;
            $field['date'] = $date;
            $insert = $action->database->insert_query("tbl_videos", $field);
            if ($insert) {
                $json['status'] = 100;
