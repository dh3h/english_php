...
if (isset($_POST['phase_name']) && !empty($action->database->text_filter($_POST['phase_name']))) {
    $phase_name = $action->database->text_filter($_POST['phase_name']);
} else {
    $json['status'] = 101;
    $json['msg'] = 'Oops!! Enter Phase Name';
}
...
if (!isset($json['status'])) {
    ...
    if (isset($_POST['hidden_id']) && !empty($action->database->text_filter($_POST['hidden_id']))) {
        ...
        if (!isset($json['status']) && empty($hidden_id)) {
            ...
            $field['date'] = $date;
            $insert = $action->database->insert_query("tbl_phase", $field);
            if ($insert) {
                $json['status'] = 100;
