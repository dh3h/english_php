...
if (isset($_GET['lesson_id']) && !empty($action->database->text_filter($_GET['lesson_id']))) {
    ...
    $select_correct_sentence_id = $action->database->query_sql("SELECT * FROM `tbl_correct_sentence` WHERE lesson_id = '{$lesson_id}' AND status = 1;");
    if($select_correct_sentence_id){
    }
} else {
...
<div class="appHeader bg-primary text-light">
  ...
  <div class="pageTitle">Find the Correct Sentence</div>
  <div class="right">
  </div>
</div>
<!-- * App Header -->
...
<div id="appCapsule">
  <div class="container">
    <!-- start Quiz button -->
    <div class="start_btn"><button type="button" class="btn btn-outline-primary rounded shadowed mr-1 mb-1">Start</button></div>
    <!-- <div class=""><button>Start Quiz</button></div> -->
...
?>
<script>
  const convertToNewFormat = (obj) => {
    const config = JSON.parse(obj.config);
    const correctAnswer = config.ans;
    const options = Object.values(config).filter(val => val !== correctAnswer);

    return {
      numb: Number(correctAnswer),
      question: obj.question.replace(/<\/?p>/g, ''),
      answer: options[Number(correctAnswer) - 1],
      options: options.map(option => option.replace(/^\w/, c => c.toUpperCase()))
    };
  };

  answer_the_questions_list = <?= json_encode($select_correct_sentence_id); ?>;
