...
if (isset($_GET['lesson_id']) && !empty($action->database->text_filter($_GET['lesson_id']))) {
    ...
    $select_rearragment_id = $action->database->query_sql("SELECT * FROM `tbl_rearrangements` WHERE lesson_id = '{$lesson_id}' AND status = 1;");
    if ($select_rearragment_id) {
    }
} else {
...
<script>
    ...
    rearrangements_list = <?= json_encode($select_rearragment_id) ?>;
    // console.log(rearrangements_list);
    const questions = rearrangements_list.map(item => {
        const question = item.questions;
        const items = item.rearragments_text.split(' ');
